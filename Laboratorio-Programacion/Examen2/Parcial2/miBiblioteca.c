...
*/
void cargarPelicula(ArrayList* this){
...
void agregarListaNegra(ArrayList* this){
    FILE* fp;
    fp = fopen("black_list.csv","r");
    if(fp==NULL){
        printf("Error!!! no se pudo abrir el archivo");
        return 0;
    }
    cargarCorreo(this,fp);
    fclose(fp);
    return 1;
}
...
EDireccionesEmail* obtenerLinea(EDireccionesEmail* correo, FILE* mail){

    correo = NULL;
    while(!feof(mail)){
        fread(correo,sizeof(EDireccionesEmail),1,mail);
        fscanf(mail,"%[^,],%[^]\n",correo->nombre,correo->email);
    }
    return correo->email;
}

int cargarCorreo(ArrayList* this, FILE* archivoMails){
    int sizeList = al_len(this)-1;
    int i;
    void* correoParseado;
    int returnAux = 0;
    for(i=0;i<sizeList;i++){
        correoParseado=obtenerLinea(this->pElements[i],archivoMails);
        if(correoParseado == NULL){
            printf("Error en el parseado de archivos");
            returnAux = -1;
            break;
        }
        al_set(this,i,correoParseado);
    }
    return returnAux;
}
